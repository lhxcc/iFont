...
const createTtf = (svgFile, outputDir) => {
  ...
  const ttfFile = path.resolve(outputDir, `${config.id}.ttf`);
  fs.writeFile(ttfFile, ttf, function (err) {
    ...
    const woffFile = path.resolve(outputDir, `${config.id}.woff`);
    const woff2File = path.resolve(outputDir, `${config.id}.woff2`);
    const eotFile = path.resolve(outputDir, `${config.id}.eot`);
...
const generateFont = (inputDir, outputDir, done) => {
  config.charmap = [];
  ScanDir(inputDir);

  createSvg(inputDir, config, function(err, result) {
    const file = path.resolve(outputDir, `${config.id}.svg`);
