...
const createDemoHtml = (outputDir) => {
  ...
  fs.readFile(path.join(__dirname, CSS_TEMPLATE_FILE), 'utf-8', (err, template) => {
    ...
    const cssFile = path.resolve(outputDir, config.fontfileName + '.css');
...
const generateFont = (downList, outputDir, done) => {
  const list = JSON.parse(downList);
  config.charmap = [];
  config.fontfileName = 'iconfont';
  ScanList(list);
  createSvg(config, function(err, result) {
